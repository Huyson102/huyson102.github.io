...
function listenConversation(){
    db.collection("conversations").where("list_users","array-contains", user.authedUser.email)
    .onSnapshot(function (snapshot) {
        const conversations = snapshot.docChanges();
        for (let i = 0; i < conversations.length; i++) {
            const con = conversations[i].doc.data();
            if(conversations[i].type === "added" 
            || (conversations[i].type === "modified" && addedconversations.index0f(conversations[i].doc.id) <0 )){
...
function updateListUser(conId) {
    ...
    .catch(function(err){
        console.err("fail to fetch conversation: ", err)
    })
}
...
// export {addUser, activeConversation, saveConversation, uppdateActiveCon}
